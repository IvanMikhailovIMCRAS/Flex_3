...
!**************************************************************************************
    ...
    open(n_input, file = name_input, status = 'old', iostat=ioer)
        ...
        read(n_input,*,iostat=ioer) L1
        if (ioer.ne.0) call ERROR(22)
        read(n_input,*,iostat=ioer) Uext1
        ...
        read(n_input,*,iostat=ioer) L2
        if (ioer.ne.0) call ERROR(24)
        read(n_input,*,iostat=ioer) Uext2
        if (ioer.ne.0) call ERROR(25)
    ...
    if (L1.gt.n_layer.or.L2.gt.n_layer) call ERROR(25)
